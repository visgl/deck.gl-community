...
| `@deck.gl-community/timeline-layers` | `TimeAxisLayer` | ✅ | ✅ | Dual-backend line ticks; WebGPU automatically uses the shared packed `FastTextLayer`. |
...
| Trace rendering | `TraceGraphLayer`, `TracePreparedStateLayer`, `TraceProcessLayer`, counter sparklines, and `TimeAxisLayer` | Reuse shared dual-backend blocks, fast text, and lines; preserve external float32 trace attributes; automatically select portable text and straight dependency routes on WebGPU. |
