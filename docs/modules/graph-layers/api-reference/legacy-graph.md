...
`LegacyGraph` is a mutable in-memory implementation of the [`Graph`](./graph.md) interface. It is the
backing structure used by existing layouts and widgets that expect the original deck.gl graph API.

## Storage strategy

LegacyGraph stores [`Node`](./node.md) and [`Edge`](./edge.md) class instances directly. Each node or
...
- `LegacyGraph` is ideal when your pipeline already produces `Node` and `Edge` instances or when you
