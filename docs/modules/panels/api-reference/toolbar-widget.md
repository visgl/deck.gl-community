...
Use it when an example or app needs a small control strip without building a custom React toolbar. `ToolbarWidget` is generic and lives in `@deck.gl-community/panels`, unlike edit-mode-specific widgets from `@deck.gl-community/editable-layers`.

## Usage

```tsx
import {ToolbarWidget} from '@deck.gl-community/panels';
