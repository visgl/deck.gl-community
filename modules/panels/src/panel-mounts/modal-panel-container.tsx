...
import {KeyboardShortcutsManager} from '../lib/keyboard-shortcuts/keyboard-shortcuts-manager';
import {
  PanelContainer,
...
export type ModalPanelContainerDialogPlacement = 'center' | 'left';

/** Controls whether the modal blocks pointer interaction outside the dialog. */
export type ModalPanelContainerPresentation = 'modal' | 'floating';

/**
 * Props for {@link ModalPanelContainer}.
 */
export type ModalPanelContainerProps = PanelContainerProps & {
...
function isModalDragHandleTarget(target: Element, dragHandleSelector?: string): boolean {
  ...
  return Boolean(target.closest('[data-modal-panel-container-header="true"]'));
}
...
function ModalPanelContainerView({
  panel,
  title,
  hideTrigger,
  triggerLabel,
  ...
  const handleDialogClick = (event: JSX.TargetedMouseEvent<HTMLDivElement>) => {
    ...
    if (event.target.closest('[data-modal-panel-container-close="true"]')) {
      onOpenChange(false);
    }
  };
      ...
      {open && (
        <>
          ...
          <div style={wrapperStyle}>
            ...
            >
              {showTitleBar ? (
                <div data-modal-panel-container-header="true" style={MODAL_HEADER_STYLE}>
...
export class ModalPanelContainer extends PanelContainer<ModalPanelContainerProps> {
  ...
  #setOpenProps(props: Partial<ModalPanelContainerProps>): void {
