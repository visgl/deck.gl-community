...
import {PanelComponent, type PanelPlacement} from './panel-component';
...
export type PanelManagerProps = {
  /**
   * Root HTML element that receives placement hosts and panel component DOM.
   */
  parentElement: HTMLElement;
  /**
   * Optional deck instance used to forward redraw, viewport, and event hooks
   * for panel components that can take advantage of deck runtime state.
   */
  deck?: unknown | null;
  /**
 ...
 * `WidgetManager`: it creates placement hosts, reconciles mounted
...
export class PanelManager {
  ...
  deck?: unknown | null;
  /**
   * Root HTML element that owns this host's placement hosts.
   */
  parentElement: HTMLElement;

  private className?: string;
  private defaultComponents: PanelComponent[] = [];
  private components: PanelComponent[] = [];
  private resolvedComponents: PanelComponent[] = [];
   ...
   */
  setProps(props: {components?: (PanelComponent | null | undefined)[]}) {
    if (props.components && !areComponentListsEqual(props.components, this.components)) {
      const nextComponents = props.components.filter(isPanelComponent);
      this._setComponents(nextComponents);
    }
  }

  /**
   * Removes all mounted instances and internal placement hosts.
   */
  finalize() {
   ...
   */
  addDefault(component: PanelComponent) {
  ...
  /**
   * Forwards redraw and viewport notifications to mounted panel components.
   *
  ...
  /**
   * Forwards hover events to panel components whose `viewId` matches the hovered view.
   */
  ...
  /**
   * Forwards one gesture event family to panel components whose `viewId` matches the
   * active view.
   */
  onEvent(
  ...
  private _setComponents(nextComponents: PanelComponent[]) {
    const oldComponentMap: Record<string, PanelComponent | null> = {};
    ...
    for (let component of nextComponents) {
      ...
      if (!oldComponent) {
        ...
        oldComponent.constructor !== component.constructor ||
        oldComponent.viewId !== component.viewId ||
        oldComponent.placement !== component.placement ||
        oldComponent._container !== component._container
      ) {
  ...
  private _addComponent(component: PanelComponent) {
    const {viewId = null, placement = DEFAULT_PLACEMENT} = component;
    const container = component._container ?? viewId;
  ...
  private _removeComponent(component: PanelComponent) {
    component.onRemove?.();
  ...
  private _getContainer(
    viewIdOrContainer: string | HTMLElement | null,
...
function areComponentListsEqual(
  left: readonly (PanelComponent | null | undefined)[],
  right: readonly PanelComponent[]
): boolean {
...
function isPanelComponent(
  component: PanelComponent | null | undefined
): component is PanelComponent {
  return component instanceof PanelComponent;
}
