...
import {AccordeonPanelContainer, AccordeonPanel} from './accordeon-panel';
import {ColumnPanelContainer, ColumnPanel} from './column-panel';
import {SplitterPanel} from './splitter-panel';
import {TabbedPanelContainer, TabbedPanel} from './tabbed-panel';
import {CustomPanel} from '../leaf-panels/custom-panel';
...
describe('composite panels', () => {
  it('toggles accordion panels with uncontrolled multi-expand behavior and emits panel ids', async () => {
    ...
    render(
      h(AccordeonPanelContainer, {
        panels: [
          new TestPanel({id: 'first', title: 'First', content: createPanelContent('first')}),
          new TestPanel({id: 'second', title: 'Second', content: createPanelContent('second')})
        ],
  ...
  it('renders child panels in order through a column container and hides empty titles', () => {
    ...
    render(
      h(ColumnPanelContainer, {
        panels: [
          new TestPanel({id: 'overview', title: '', content: createPanelContent('overview')}),
          new TestPanel({id: 'actions', title: 'Actions', content: createPanelContent('actions')})
        ]
      }),
      root
    );
  ...
  it('wraps tabbed panels from an ordered panel array', () => {
    ...
    const panel = new TabbedPanel({
      ...
      panels: [
        new TestPanel({id: 'b', title: 'B', content: <div>B content</div>}),
        new TestPanel({id: 'a', title: 'A', content: <div>A content</div>})
      ]
    });
  ...
  it('wraps column panels from an ordered panel array', () => {
    ...
    const panel = new ColumnPanel({
      ...
      title: 'Column Panels',
      panels: [
        new TestPanel({id: 'overview', title: '', content: <div>overview content</div>}),
        new TestPanel({id: 'actions', title: 'Actions', content: <div>actions content</div>})
      ]
    });
  ...
  it('wraps split panels with the first panel in the first pane and remaining panels in order', () => {
    ...
    const panel = new SplitterPanel({
      ...
      panels: [
        new TestPanel({id: 'first', title: 'First', content: createPanelContent('first')}),
        new TestPanel({id: 'second', title: 'Second', content: createPanelContent('second')}),
        new TestPanel({id: 'third', title: 'Third', content: createPanelContent('third')})
      ]
    });
  ...
  it('renders one splitter panel without a handle and renders empty input as empty content', () => {
    ...
    const singlePanel = new SplitterPanel({
      panels: [new TestPanel({id: 'only', title: 'Only', content: createPanelContent('only')})]
    });
  ...
  it('supports horizontally scrollable tab rows when requested', () => {
    ...
    render(
      h(TabbedPanelContainer, {
        ...
        panels: [
          new TestPanel({id: 'one', title: 'One', content: createPanelContent('one')}),
          new TestPanel({id: 'two', title: 'Two', content: createPanelContent('two')}),
          new TestPanel({id: 'three', title: 'Three', content: createPanelContent('three')})
        ]
      }),
      root
    );
  ...
  it('forces explicit light and dark theme overrides', async () => {
    ...
    const panel = new ColumnPanel({
      ...
      title: 'Forced themes',
      panels: [
        new TestPanel({id: 'light', title: 'Light', theme: 'light', content: <div>light</div>}),
        new TestPanel({id: 'dark', title: 'Dark', theme: 'dark', content: <div>dark</div>})
      ]
    });
    renderPanel(panel, root);
    await Promise.resolve();
