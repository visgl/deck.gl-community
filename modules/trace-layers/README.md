...
`TraceEngine` owns mounted trace-view interaction, collapse, layout, prepared-scene, and diagnostics
state below React. `DeckTraceGraph` renders one mounted engine as the full React trace viewer.
Custom deck.gl shells can render normalized graphs directly with `TraceGraphLayer`, render
`TraceChunkStore` windows with `TraceStoreLayer`, or render already-prepared trace view state with
