...
/**
 * Parses a streamed Chrome trace payload into typed Arrow record batches of raw events.
 */
...
/**
 * Reads Chrome trace file metadata from one Arrow schema metadata map.
 */
export function parseChromeTraceArrowSchemaMetadata(metadata: Map<string, string>): {
  /** Optional top-level display time unit stored on the Arrow schema. */
  displayTimeUnit?: string;
  /** Optional top-level metadata stored on the Arrow schema. */
...
/**
 * Emits a sequence of Arrow batches from one complete event list.
 */
...
/**
 * Builds the passthrough JSON payload for unmodeled Chrome trace fields.
 */
...
/**
 * Normalizes the Chrome trace Arrow batch size.
 */
