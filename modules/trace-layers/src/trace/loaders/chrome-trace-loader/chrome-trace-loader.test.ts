...
/**
 * Builds a compact Chrome trace fixture that exercises the Arrow loader edge cases.
 */
...
/**
 * Encodes one Chrome trace fixture into an ArrayBuffer for parser coverage.
 */
...
describe('chrome trace Arrow parser', () => {
  ...
  it('parses a full Chrome trace into an Arrow table with encoded args and schema metadata', () => {
    ...
    const table = parseChromeTraceToArrowTable(encodeChromeTraceFixture(traceFile)) as arrow.Table;
  ...
  it('attaches top-level metadata to the final streamed Arrow batch', async () => {
    const traceFile = createChromeTraceFixture();
    ...
    expect(batches).toHaveLength(2);
    expect(batches[1]?.schema.metadata.get('chromeTrace.displayTimeUnit')).toBe('us');
