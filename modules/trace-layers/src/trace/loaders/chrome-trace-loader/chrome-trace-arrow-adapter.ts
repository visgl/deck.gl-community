...
/**
 * One Arrow-backed Chrome trace source item accepted by the streaming helpers.
 */
...
export function readChromeTraceArrowSourceMetadata(source: ChromeTraceArrowSourceItem): {
  ...
  /** Optional top-level metadata object stored on the Arrow schema. */
...
/**
 * Restores one required string-or-number id-like value from Arrow and passthrough JSON.
 */
...
/**
 * Restores one optional string-or-number id-like value from Arrow and passthrough JSON.
 */
...
/**
 * Parses one optional JSON string stored in Arrow columns or schema metadata.
 */
