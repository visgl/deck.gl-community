/**
 * Returns the visible row index for one local access if it is a valid integer index.
 */
...
/**
 * Returns the visible row index for one relative `.at()`-style access.
 */
...
/**
 * Normalizes one list of raw Arrow row indexes into an owned `Int32Array`.
 *
...
export function normalizeIndexedArrowIndexes(
  ...
  for (let rowIndex = 0; rowIndex < normalizedIndexes.length; rowIndex += 1) {
