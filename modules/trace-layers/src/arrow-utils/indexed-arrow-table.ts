...
/**
 * Readonly indexed row/column view over one backing Arrow table.
 *
 * @typeParam T - Backing Arrow column map shared with the source table.
 */
export class IndexedArrowTable<T extends arrow.TypeMap> {
   ...
   *
   * @param rowIndex - Visible row index within the indexed view.
   * @returns Materialized Arrow row for the visible row index, or `null` when the index is
   * invalid.
   */
  ...
  getTemporaryRow(rowIndex: number): IndexedArrowTableRow<T> | null {
    ...
    for (const field of this.schema.fields) {
      ...
      mutableTemporaryRow[columnName] = childVector
        ? ((childVector.get(rawIndex) as T[typeof columnName]['TValue'] | null) ?? undefined)
        : undefined;
    }
   ...
   * @returns Materialized Arrow row for the resolved visible row index, or `null` when the index
   * is invalid.
   */
  ...
  concat(...others: IndexedArrowTable<T>[]): IndexedArrowTable<T> {
    ...
    const concatenatedTable = this.table.concat(
      ...others.map(sourceTable => sourceTable.table)
    ) as arrow.Table<T>;
    ...
    for (const sourceTable of sourceTables) {
      for (let rowIndex = 0; rowIndex < sourceTable.numRows; rowIndex += 1) {
        concatenatedIndexes[viewOffset + rowIndex] =
          sourceTable.indexes[rowIndex]! + tableRowOffset;
      }
   ...
   *
   * @returns Arrow table containing the current visible rows in indexed order, including
   * duplicates when present.
   */
  materializeArrowTable(): arrow.Table<T> {
    reportIndexedArrowMaterialization(this);
   ...
   *
   * @returns Iterator over materialized visible rows in indexed order.
   */
...
>();
let indexedArrowMaterializationCount = 0;
...
/**
 * Reports one indexed Arrow materialization through probe logging so repeated table snapshots are
 * visible during UI debugging.
 */
function reportIndexedArrowMaterialization<T extends arrow.TypeMap>(
  table: IndexedArrowTable<T>
): void {
  indexedArrowMaterializationCount += 1;

  log.probe(0, 'IndexedArrowTable.materializeArrowTable', {
    count: indexedArrowMaterializationCount,
    numRows: table.numRows,
    numCols: table.numCols,
    sourceRows: table.table.numRows
  })();
}
