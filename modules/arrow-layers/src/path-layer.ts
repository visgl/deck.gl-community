...
const {data: _data, getPath: _getPath, ..._defaultProps} = PathLayer.defaultProps;
 ...
 */
export class GeoArrowPathLayer<ExtraProps extends {} = {}> extends CompositeLayer<
...
> {
  ...
  renderLayers(): Layer<{}> | LayersList | null {
    ...
    if (this.props.getPath !== undefined) {
      ...
      throw new Error('getPath should be an arrow Vector of LineString or MultiLineString type');
    } else {
      const lineStringVector = getGeometryVector(table, EXTENSION_NAME.LINESTRING);
      if (lineStringVector !== null) {
        return this._renderLayersLineString(lineStringVector);
      }

      const multiLineStringVector = getGeometryVector(table, EXTENSION_NAME.MULTILINESTRING);
