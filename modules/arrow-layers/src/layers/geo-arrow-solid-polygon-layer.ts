...
const ourDefaultProps: Pick<
  ...
  '_normalize' | '_windingOrder' | '_validate' | 'earcutWorkerUrl' | 'earcutWorkerPoolSize'
> = {
...
export class GeoArrowSolidPolygonLayer<ExtraProps extends {} = {}> extends CompositeLayer<
...
> {
  ...
  initializeState(_context: LayerContext): void {
    this.state = {
      ...
      triangles: null,
      earcutWorkerRequest:
        this.props.earcutWorkerUrl === null || this.props.earcutWorkerUrl === undefined
          ? null
  ...
  async initEarcutPool(): Promise<Pool<FunctionThread> | null> {
    ...
    try {
      const pool = Pool<FunctionThread>(() => spawn(BlobWorker.fromText(workerText)), 8);
  ...
  async _updateEarcut(table: arrow.Table): Promise<Uint32Array[]> {
    ...
    const MultiPolygonVector = getGeometryVector(table, EXTENSION_NAME.MULTIPOLYGON);
  ...
  async _earcutPolygonVector(geometryColumn: ga.vector.PolygonVector): Promise<Uint32Array[]> {
    ...
    for (let recordBatchIdx = 0; recordBatchIdx < geometryColumn.data.length; recordBatchIdx++) {
      ...
      pool.queue(async (earcutWorker) => {
        const earcutTriangles = await earcutWorker(Transfer(preparedPolygonData, arrayBuffers));
  ...
  _earcutPolygonVectorMainThread(geometryColumn: ga.vector.PolygonVector): Uint32Array[] {
    const result: Uint32Array[] = new Array(geometryColumn.data.length);
  ...
  _earcutMultiPolygonVectorMainThread(geometryColumn: ga.vector.MultiPolygonVector): Uint32Array[] {
    const result: Uint32Array[] = new Array(geometryColumn.data.length);
  ...
  renderLayers(): Layer<{}> | LayersList | null {
    ...
    if (this.props.getPolygon !== undefined) {
      ...
      throw new Error('getPolygon should be an arrow Vector of Polygon or MultiPolygon type');
    } else {
      const polygonVector = getGeometryVector(table, EXTENSION_NAME.POLYGON);
      if (polygonVector !== null) {
  ...
  _renderLayersPolygon(geometryColumn: ga.vector.PolygonVector): Layer<{}> | LayersList | null {
    const {table} = this.state;
    if (!table) return null;
