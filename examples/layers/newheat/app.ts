...
export function mountNewHeatExample(container: HTMLElement): () => void {
  ...
  root.tabIndex = -1;
  root.innerHTML = `<div class="nh-stage" aria-label="Interactive burning path visualization"></div>
    <div class="nh-recording-status" hidden><span role="status"></span><button type="button">Cancel</button></div>`;
  container.appendChild(root);
  const stage = root.querySelector<HTMLDivElement>('.nh-stage')!;
  const status = root.querySelector<HTMLDivElement>('.nh-recording-status')!;
  let settings = {
    currentTime: 195,
    trailLength: 160,
    width: 18,
