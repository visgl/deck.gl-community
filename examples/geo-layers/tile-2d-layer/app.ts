...
import {Tile2DLayer, Tile2DTileset, TileGridLayer} from '../../../modules/geo-layers/src';
...
type Tile2DLayerExampleOptions = {
  mode?: 'full' | 'compact';
};
...
const THEME_WIDGET_STYLE = `
  .tile-2d-layer .deck-widget.deck-widget-theme .deck-widget-button {
    width: 30px;
    height: 30px;
    display: flex;
    align-items: center;
  ...
  .tile-2d-layer .deck-widget.deck-widget-theme .deck-widget-button > button {
    width: 28px;
    height: 28px;
    padding: 0;
    border: none;
  ...
  .tile-2d-layer .deck-widget.deck-widget-theme .deck-widget-icon {
    display: block;
    width: 100%;
    height: 100%;
  ...
  .tile-2d-layer .deck-widget.deck-widget-theme button.deck-widget-sun .deck-widget-icon {
  ...
  .tile-2d-layer .deck-widget.deck-widget-theme button.deck-widget-moon .deck-widget-icon {
  ...
  .tile-2d-layer .deck-widget.deck-widget-theme {
    margin-top: 12px;
  }
`;
...
function buildInfoPanel(tileset: Tile2DTileset<TileContent>, hoveredId: string | null, selectedId: string | null) {
  return new ColumnWidgetPanel({
    id: 'tile-2d-layer-panel',
    title: '',
    panels: {
      overview: new MarkdownWidgetPanel({
        id: 'tile-2d-layer-overview',
        title: '',
        markdown: [
          'One shared auto-tiled GeoJSON `TableTileSource` and one shared `Tile2DTileset` feed styled `Tile2DLayer` comparisons plus a minimap.',
          `Hovered feature: **${hoveredId ?? 'none'}**`,
          `Selected feature: **${selectedId ?? 'none'}**`
        ].join('\n\n')
      }),
      stats: new AccordeonWidgetPanel({
        id: 'tile-2d-layer-stats-accordion',
        title: '',
        panels: {
          stats: new StatsWidgetPanel({
            id: 'tile-2d-layer-stats',
            title: 'Stats',
            stats: tileset.stats,
            statNames: [
              'Tiles In Cache',
...
export function mountTile2DLayerExample(
  container: HTMLElement,
  options: Tile2DLayerExampleOptions = {}
): () => void {
  const mode = options.mode || 'full';
  const root = container.ownerDocument.createElement('div');
  root.className = 'tile-2d-layer';
